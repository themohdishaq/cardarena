...
export class VoiceManager extends EventTarget {
  ...
  async init() {
    try {
      ...
      this.localStream = this._createSilentStream();
      this.dispatchEvent(new CustomEvent("permission_denied"));
    }

    this.silentTrack = this._createSilentStream().getAudioTracks()[0];
  ...
  setMic(on) {
    ...
    if (on) {
      // Re-enable the real mic track in all connections
      ...
      // Replace with silence in all connections
      for (const peerId in this.connections) {
        const pc = this.connections[peerId];
        const sender = pc.getSenders().find((s) => s.track?.kind === "audio");
  ...
  _createPeerConnection(peerId) {
    ...
    if (this.localStream) {
      this.localStream.getTracks().forEach((track) => {
        pc.addTrack(this.micOn ? track : this.silentTrack, this.localStream);
      });
    }
  ...
  async _createOffer(peerId) {
    ...
    this.socket.emit("webrtc_offer", {
      ...
      roomId: null, // roomId not needed for direct peer signal
    });
  }
